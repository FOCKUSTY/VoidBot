const
  ...
  { color, bold, random } = require('colors'),
  { shuffle } = require('./shuffle'),
  { ActivityType } = require('discord.js'),
  { getActivities, funcKristyAct } = require('./updatejson'),
  ...
  { checkKristyStatus } = require('./activity'),
  { kristyGuildId, kristyId, authorId } = require('../../config.json'),
...
const setRandomnessActivity = (client, textActivity=false, log=true) => 
{
    try
    {
      ...
      if(randomAct.indexOf('${rAnimeTitle}') != -1)
      {
        ...
        const rAnimeTitle = animeNames[pseudoRandomNumber(0, animeNames.length-1, 1, 2, randomAnimeTitles, null, null, true, true)];
